    ...
    <v-card :loading="permissionsDialogLoading || permissionsCatalogLoading">
      ...
      <v-card-text class="pb-4">
        <div class="text-body-2 mb-4 text-medium-emphasis">
          ...
          <span v-if="permissionsRole?.description"> — {{ permissionsRole.description }}</span>
        </div>
...
<script lang="ts" setup>
  ...
  async function openPermissionsDialog (role: Role) {
    ...
    try {
      ...
      const response = await httpClient.get<unknown>(`/api/v1/roles/${role.id}/permissions`)
      ...
      error.value = error_ instanceof Error ? error_.message : 'No fue posible cargar los permisos del rol'
    } finally {
  ...
  async function saveRolePermissions () {
    ...
    const roleId = permissionsRole.value.id
    ...
    try {
      ...
      await Promise.all(
        toRemove.map(permissionId =>
          httpClient.delete<unknown>(`/api/v1/roles/${roleId}/permissions/${permissionId}`),
        ),
      )

      closePermissionsDialog()
    } catch (error_) {
      error.value = error_ instanceof Error ? error_.message : 'No fue posible guardar los permisos del rol'
    } finally {
      permissionsDialogLoading.value = false
    }
  }
